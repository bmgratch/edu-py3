...
def getNewBoard():
    #create a board 50 x 15)
    board = []
    for x in range(60): # The main list is a list of 60 lists
        board.append([])
        for y in range(15): # Each list in the main list has 15 single-char strings
            #Use different characters for ocean to make it mor readable
...
def drawBoard(board):
    ...
    tensDigitsLine = '    ' # Initial space for numbers down the left
    for i in range(1, 6):
    ...
    for row in range(15):
        # Single digit numbers need to be badded with an extra space
        if row < 10:
            extraSpace = ' '
        else:
            extraSpace = ''
            
        # Create the string for this row based on the board.
        boardRow = ''
...
def getRandomChests(numChests):
    # Create a list of chest data structiures ( two item lists of x, y coordintes)
    chests = []
    while len(chests) < numChests:
        newChest = [random.randint(0, 59), random.randint(0,14)]
        if newChest not in chests: # make sure it's not already here
            chests.append(newChest)
    return chests

def isOnBoard(x, y):
    # Return True if coordinates are on the board; otherwise, return False
    return x >= 0 and x <= 59 and y >= 0 and y <= 14

def makeMove(board, chests, x, y):
    # Change the board data structure with sonar device character
    # Remove Treasure chests from the chests list as they are found.
    # Retyrn False if the move is invalid
    # Otherwise, return the string of the result of this move.
    smallestDistance = 100 # Any chest will be closer than 100
    for cx, cy in chests:
        distance = math.sqrt((cx - x) ** 2 + (cy - y) ** 2)

        if distance < smallestDistance: # We want the closest chest.
    ...
    if smallestDistance == 0:
        ...
        if smallestDistance < 10:
            ...
            return 'Treasure chest detected at a distance of %s fromt he sonar device.' % smallestDistance
        else:
            board[x][y] = 'X'
            return 'Sonar did not detect anything. All treasure chests are out of range.'

def enterPlayerMove(previousMoves):
    # Let the player enter their move. Return a two-item list of xy coordinates
    print('Where do you want to drop the next sonar device? (0-59 0-15) (or type quit)')
    while True:
        move = input()
        if move.lower() == 'quit':
...
while True:
    ...
    while sonarDevices > 0:
        ...
        previousMoves.append([x, y])    # We must track all the moves so the sonar devices can be updated.
    ...
    if sonarDevices == 0:
        ...
        print('head for home with treasure chests still out there! Game over.')
